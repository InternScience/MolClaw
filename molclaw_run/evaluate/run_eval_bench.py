...
def _aggregate_mol_edit_scores(results_out: dict) -> None:
    keys = [key for key in results_out if key.startswith("molbench-mo-edit_") and key != "molbench-mo-edit_overall"]
    if not keys:
        return
    ...
    results_out["molbench-mo-edit_overall"] = {
...
def main() -> None:
    ...
    if dataset == "ChemCoTBench":
        work_space = os.path.dirname(os.path.dirname(script_dir))  # shy_local
        if args.eval_root:
            ...
            if isinstance(p, str) and p.strip():
                ...
                eval_root = os.path.abspath(os.path.join(work_space, "bench", "ChemCoTBench", "baseline_and_eval"))

    from MolClaw.molclaw_run.evaluate.eval_runner import (
